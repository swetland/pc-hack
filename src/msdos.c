...
char let;
{
	...
	/* buffer is not checked for overflow.
	 */
	static char buf[100];
	...
	if (pos == NULL)
		pos = obj_symbols;
	if (!US || !UE)
		return names[pos - obj_symbols];
	else {
		Sprintf(buf, "%s%s%s", US, names[pos - obj_symbols], UE);
		return buf;
	}
}
...
read_config_file() {
	...
	&&  (fp = fopenp(configfile, "r", NULL)) == NULL) {
