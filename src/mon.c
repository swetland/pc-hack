...
movemon()
{
	...
	if(warnlevel > lastwarnlev || moves > lastwarntime + 5){
	    register char *rr;
# ifdef MSDOS
	    if (Warning & LEFT_RING && Warning & RIGHT_RING)
		rr = "Both your rings glow";
	    else if (Warning & RIGHT_RING)
		rr = "Your right ring glows";
	    else if (Warning & LEFT_RING)
...
register struct monst *mtmp;
{
	...
	if(!nearby ||
		...
		/* A bug which prevented fast monsters from hitting you when
		 * they caught up to you. -dgk
		 */
		nearby = (dist(mtmp->mx, mtmp->my) < 3);
		scared = (nearby && onscary);
		if(scared && !mtmp->mflee) {
